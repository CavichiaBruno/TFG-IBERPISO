...
class UserPropertyController extends Controller
{
    ...
    {
        // 1. Los datos ya están validados por StorePropertyRequest
        ...
        $property->destacada = \DB::raw('false'); // Por defecto no destacada
    ...
    public function update(UpdatePropertyRequest $request, $id)
    {
        ...
        // 2. Los datos ya están validados por UpdatePropertyRequest
        $validated = $request->validated();
        ...
        $booleanFields = ['tiene_ascensor', 'tiene_parking', 'tiene_terraza', 'tiene_jardin', 'tiene_piscina', 'aire_acondicionado', 'destacada', 'activa'];
        foreach ($booleanFields as $field) {
            if ($request->has($field)) {
                $property->$field = \DB::raw('true');
            } else {
                // Solo actualizamos si el campo viene en el request (para evitar borrar valores si no se enviaron)
                // O mejor: en una actualización de perfil de propiedad, si el checkbox no está, es false.
                $property->$field = \DB::raw('false');
            }
        }

        // 5. Gestión del Certificado Energético (Reemplazo o Eliminación)
        if ($request->boolean('eliminar_certificado')) {
            ...
            $property->certificado_energetico_archivo = null;
