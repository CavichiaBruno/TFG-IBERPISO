...
@section('topbar-actions')
    <a href="{{ route('properties.show', [$property->id, $property->slug]) }}" target="_blank" class="btn btn-outline">Ver en portal</a>
@endsection
    ...
    <div class="form-section">
        ...
        <div class="media-section">
            ...
            <div class="upload-zone" id="image-upload-zone" data-type="image" data-property="{{ $property->id }}">
                <svg viewBox="0 0 24 24" width="40" height="40"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" stroke="currentColor" fill="none" stroke-width="1.5"/><polyline points="17 8 12 3 7 8" stroke="currentColor" fill="none" stroke-width="1.5"/><line x1="12" y1="3" x2="12" y2="15" stroke="currentColor" stroke-width="1.5"/></svg>
                ...
                <input type="file" id="image-input" multiple accept="image/jpeg,image/png,image/webp" class="upload-input" style="display:none">
            </div>
                ...
                @foreach($property->media->where('file_type', 'image') as $img)
                    <div class="media-item" data-id="{{ $img->id }}">
                        ...
                        <div class="media-actions">
                            <button type="button" class="set-cover-btn {{ $img->is_cover ? 'cover-active' : '' }}" data-id="{{ $img->id }}" title="Establecer como portada">★</button>
                            <button type="button" class="delete-media-btn" data-id="{{ $img->id }}" title="Eliminar">✕</button>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="media-section">
            ...
            <div class="upload-zone upload-zone-sm" id="pdf-upload-zone" data-type="pdf" data-property="{{ $property->id }}">
                <svg viewBox="0 0 24 24" width="32" height="32"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" stroke="currentColor" fill="none" stroke-width="1.5"/><polyline points="14 2 14 8 20 8" stroke="currentColor" fill="none" stroke-width="1.5"/></svg>
                <p><label for="pdf-input" class="upload-link">Subir PDF</label></p>
                <small>PDF, DOCX — máx. 10 MB</small>
                <input type="file" id="pdf-input" accept=".pdf,.docx,application/pdf" class="upload-input" style="display:none">
            </div>
            <div id="docs-list">
