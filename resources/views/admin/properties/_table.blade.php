        ...
        <tr data-id="{{ $p->id }}">
            ...
            <td>
                <div class="table-property-cell">
                    ...
                    <span title="{{ $p->title }}">{{ Str::limit($p->title, 40) }}</span>
                </div>
            </td>
            <td>{{ $p->city }}</td>
            <td class="price-cell">€{{ $p->formatted_price }}</td>
            <td><span class="type-badge">{{ ucfirst($p->property_type) }}</span></td>
            <td><span class="op-badge op-{{ $p->operation_type }}">{{ ucfirst($p->operation_type) }}</span></td>
            <td>
                <span class="featured-star {{ $p->is_featured ? 'featured-yes' : '' }}" title="{{ $p->is_featured ? 'Destacada' : 'No destacada' }}">★</span>
            </td>
            <td>
                <button class="toggle-active-btn {{ $p->is_active ? 'active' : 'inactive' }}"
                    data-id="{{ $p->id }}" title="{{ $p->is_active ? 'Desactivar' : 'Activar' }}">
