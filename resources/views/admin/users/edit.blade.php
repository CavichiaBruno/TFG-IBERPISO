...
<div id="user-loading-overlay" style="opacity: 0; pointer-events: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(245, 245, 247, 0.8); backdrop-filter: saturate(180%) blur(20px); -webkit-backdrop-filter: saturate(180%) blur(20px); z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center; transition: all 0.3s;">
    ...
    <h3 style="margin-top: 20px; font-family: var(--font-display); font-weight: 600; font-size: 28px; line-height: 1.14; letter-spacing: 0.196px; color: #1d1d1f;">Actualizando usuario...</h3>
...
<script>
// Esperar a que el script de Lottie se cargue completamente
window.addEventListener('load', function() {
    // Pequeña espera para asegurar que dotlottie-wc esté disponible
    setTimeout(function() {
    ...
    <form id="user-edit-form" action="{{ route('admin.users.update', $user->id) }}" method="POST">
        ...
        <div class="form-grid-2">
            ...
            <div class="form-group">
                ...
                <input type="email" name="correo" class="form-input" value="{{ old('correo', $user->correo) }}" required>
            </div>
            
            <div class="form-group">
                <label class="form-label">Teléfono</label>
                <input type="tel" name="telefono" class="form-input" value="{{ old('telefono', $user->telefono) }}">
            </div>
            ...
            <div class="form-group form-col-2">
                <label class="form-label">Contraseña (dejar vacío para mantener actual)</label>
                <input type="password" name="contrasena" class="form-input" minlength="8">
            </div>
        </div>
